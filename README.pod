...
of the variable with the last occurrence. Currently, this module appends to the string 'var'
a number which is incremented on each write. A more flexible way to choose names
is on the todo list.
...
file. But, Octave will fail to read the file and print an error message. Optionally
adding or removing a dimension to 1-d pdls when reading and writing is on the todo list.
