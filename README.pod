...
If C<oned> is C<1> then a 1-d pdl of length n is written as a
as an C<nx1> pdl (a C<1xn> matlab variable). If C<oned> is C<2>
then the output piddle is C<1xn> and the matlab variable C<nx1>.
If C<oned> is zero (the default), then the 1-d pdl is written
